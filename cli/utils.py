...
TICKER_INPUT_EXAMPLES = "SPY, 0700.HK, BTC-USD, BTC/USDT"
...
def is_valid_ticker_input(value: str) -> bool:
    ...
    conventional crypto pairs such as ``BTC/USDT``. Empty input is allowed (it
...
def get_ticker() -> str:
    ...
    ticker = questionary.text(
        ...
        validate=lambda x: (
            ...
            or "Please enter a valid ticker symbol, e.g. AAPL, 0700.HK, BTC/USDT, GC=F."
        ),
...
def normalize_ticker_symbol(ticker: str) -> str:
    ...
    (e.g. ``BTCUSD`` / ``BTC/USDT`` -> ``BTC-USD``, ``XAUUSD`` -> ``GC=F``).
    Falls back to the plain upper-case if the data layer is unavailable.
    """
    try:
